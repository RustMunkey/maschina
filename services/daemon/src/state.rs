...
impl Metrics {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let runs_executing = IntGauge::new("daemon_runs_executing", "Agent runs currently executing")?;
        let runs_completed = IntCounter::new("daemon_runs_completed_total", "Agent runs completed successfully")?;
        let runs_failed = IntCounter::new("daemon_runs_failed_total", "Agent runs that failed")?;
        let runs_timed_out = IntCounter::new("daemon_runs_timed_out_total", "Agent runs that timed out")?;
        let scan_cycles = IntCounter::new("daemon_scan_cycles_total", "Number of scan cycles completed")?;
