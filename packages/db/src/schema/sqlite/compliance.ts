...
export const consentRecords = sqliteTable("consent_records", {
  ...
  type:      text("type", { enum: ["terms_of_service","privacy_policy","marketing","data_processing"] }).notNull(),
...
export const dataExportRequests = sqliteTable("data_export_requests", {
  id:          text("id").primaryKey(),
  userId:      text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  status:      text("status", { enum: ["pending","processing","ready","expired"] }).notNull().default("pending"),
