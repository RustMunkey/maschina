...
use crate::{config, services::{self, Status}};
...
enum Screen {
    Launcher,
    Logs { svc: String, lines: Vec<String>, offset: usize },
}

// ── zones ─────────────────────────────────────────────────────────────────────

#[derive(PartialEq, Clone)]
enum Zone { Services, Menu }

// ── menu ──────────────────────────────────────────────────────────────────────

#[derive(Clone, PartialEq)]
enum MenuAction { Start, Stop, Logs, Agents, Usage, Models, Setup, Code, Quit }

struct MenuItem { label: &'static str, action: MenuAction, desc: &'static str }

static MENU: &[MenuItem] = &[
    MenuItem { label: "start services",   action: MenuAction::Start,   desc: "start all services in sequence"         },
    MenuItem { label: "stop services",    action: MenuAction::Stop,    desc: "gracefully stop all running services"   },
    MenuItem { label: "view logs",        action: MenuAction::Logs,    desc: "tail live log for highlighted service"  },
    MenuItem { label: "agents",           action: MenuAction::Agents,  desc: "list, deploy, and manage agents"        },
    MenuItem { label: "usage & quota",    action: MenuAction::Usage,   desc: "view token usage and monthly limits"    },
    MenuItem { label: "models",           action: MenuAction::Models,  desc: "configure AI providers and models"      },
    MenuItem { label: "setup wizard",     action: MenuAction::Setup,   desc: "configure account, providers, database" },
    MenuItem { label: "code tool",        action: MenuAction::Code,    desc: "open the interactive code assistant"    },
    MenuItem { label: "quit",             action: MenuAction::Quit,    desc: "exit launcher (services keep running)"  },
];
...
impl App {
    ...
    fn open_logs(&mut self, svc_name: &str) {
        ...
        self.screen = Screen::Logs { svc: svc_name.to_string(), lines, offset };
    }
}
...
fn draw_launcher(f: &mut ratatui::Frame, app: &App) {
    ...
    if has_msg { constraints.push(Constraint::Length(1)); }
    ...
    // ── title ─────────────────────────────────────────────────────────────────
    let spinner    = if app.probing { SPIN[(app.tick / 2 % 4) as usize] } else { "" };
    ...
    let svc_items: Vec<ListItem> = app.services.iter().enumerate().map(|(i, svc)| {
        let cursor = svc_active && i == app.svc_sel;
        let prefix = if cursor { "▸" } else { " " };

        let (dot, dot_style) = if svc.status.is_running() {
            ("●", if svc_active { Style::default().fg(Color::White) } else { gray })
        } else {
            ("○", dim)
        };

        let name_style = if cursor { bold } else if svc.status.is_running() { gray } else { dim };
        ...
        if w >= 50 {
            spans.push(Span::styled(format!(" {}", svc.status.label()), if cursor { name_style } else { dim }));
        }
        if w >= 62 {
    ...
    let menu_items: Vec<ListItem> = MENU.iter().enumerate().map(|(i, item)| {
        let sel    = menu_active && i == app.menu_sel;
        let prefix = if sel { "▸ " } else { "  " };
        let is_quit = item.action == MenuAction::Quit;

        let label_style = if sel { bold }
            else if is_quit { dim }
            else if menu_active { gray }
            else { dim };
        ...
        if sel && w >= 32 {
            ...
            ListItem::new(Line::from(Span::styled(format!(" {}{}", prefix, item.label), label_style)))
        }
    }).collect();
    ...
    let footer = if app.zone == Zone::Services {
        if w >= 48 {
            Line::from(Span::styled(" ↑/↓ navigate  l logs  tab menu  r refresh  q quit", dim))
        } else {
            Line::from(Span::styled(" ↑/↓  l logs  tab  q", dim))
        }
    } else {
        Line::from(Span::styled(" ↑/↓ navigate  enter select  tab services  q quit", dim))
    };
...
fn draw_logs(f: &mut ratatui::Frame, svc: &str, lines: &[String], offset: usize) {
    ...
    // Header bar
    let header_rect = Rect { x: 0, y: 0, width: area.width, height: 1 };
    let log_rect    = Rect { x: 0, y: 1, width: area.width, height: area.height.saturating_sub(2) };
    let footer_rect = Rect { x: 0, y: area.height.saturating_sub(1), width: area.width, height: 1 };

    f.render_widget(Clear, area);
    f.render_widget(Paragraph::new(Line::from(vec![
        Span::styled(format!(" logs: {}", svc), bold),
    ])), header_rect);
    ...
    let shown: Vec<ListItem> = lines[start..]
        .iter()
        .take(visible)
        .map(|l| ListItem::new(Line::from(Span::styled(
            format!(" {}", l),
...
pub fn run(profile: &str) -> Result<Option<LaunchTarget>> {
    ...
    loop {
        ...
        if let Some((_, t)) = &app.msg {
            if t.elapsed() >= Duration::from_secs(5) { app.msg = None; }
        }
        ...
        match &app.screen {
            ...
            Screen::Logs { svc, lines, offset } => {
                let svc = svc.clone(); let lines = lines.clone(); let offset = *offset;
        ...
        if let Screen::Logs { lines, offset, .. } = &mut app.screen {
            ...
            match key.code {
                ...
                KeyCode::Down | KeyCode::Char('j') => { if *offset + 1 < len { *offset += 1; } }
        ...
        match key.code {
            ...
            KeyCode::Char('r') => { app.refresh(); app.set_msg("refreshed"); }

            KeyCode::Tab    => { app.zone = if app.zone == Zone::Services { Zone::Menu } else { Zone::Services }; }
            KeyCode::BackTab => { app.zone = if app.zone == Zone::Menu { Zone::Services } else { Zone::Menu }; }

            KeyCode::Up | KeyCode::Char('k') => match app.zone {
                Zone::Services => { if app.svc_sel > 0 { app.svc_sel -= 1; } }
                Zone::Menu     => { if app.menu_sel > 0 { app.menu_sel -= 1; } }
            },
            KeyCode::Down | KeyCode::Char('j') => match app.zone {
                Zone::Services => { if app.svc_sel + 1 < app.services.len() { app.svc_sel += 1; } }
                Zone::Menu     => { if app.menu_sel + 1 < MENU.len() { app.menu_sel += 1; } }
            },
            ...
            // ── menu Enter ────────────────────────────────────────────────────
            KeyCode::Enter if app.zone == Zone::Menu => {
                match MENU[app.menu_sel].action {
                    MenuAction::Quit => break,

                    MenuAction::Start => {
                        let svcs: Vec<_> = app.services.iter()
                            .filter(|s| !s.status.is_running()).cloned().collect();
                        if svcs.is_empty() {
                            app.set_msg("all services already running");
                        } else {
                            app.probing = true;
                            for (i, svc) in svcs.iter().enumerate() {
                                app.set_msg(format!("starting {} ({}/{})…", svc.name, i + 1, svcs.len()));
                    ...
                    MenuAction::Stop => {
                        ...
                        if svcs.is_empty() {
                            ...
                            app.probing = true;
                            for svc in &svcs { services::stop_svc(svc); }
                    ...
                    MenuAction::Agents => { app.exit_with = Some(LaunchTarget::Agents); break; }
                    MenuAction::Usage   => { app.exit_with = Some(LaunchTarget::Usage);  break; }
                    MenuAction::Models  => { app.exit_with = Some(LaunchTarget::Models); break; }
                    MenuAction::Setup   => { app.exit_with = Some(LaunchTarget::Setup);  break; }
                    MenuAction::Code    => { app.exit_with = Some(LaunchTarget::Code);   break; }
                }
            }
