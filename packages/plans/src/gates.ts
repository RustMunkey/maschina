...
export function getUpgradeHints(currentTier: PlanTier): UpgradeHint[] {
  ...
  if (!can.useCloudExecution(currentTier))
    hints.push({
      feature: "cloudExecution",
      requiredTier: "m1",
      description: "Run agents on Maschina cloud infrastructure",
    });
  if (!can.useMaschinaModel(currentTier))
    hints.push({
      feature: "maschinaModel",
      requiredTier: "m1",
      description: "Access the Maschina model",
    });
  ...
  if (!can.useAnalytics(currentTier))
    hints.push({
      feature: "analytics",
      requiredTier: "m5",
      description: "Usage dashboards and analytics",
    });
  if (!can.useCustomConnectors(currentTier))
    hints.push({
      feature: "customConnectors",
      requiredTier: "m5",
      description: "Build and publish custom connectors",
    });
  if (!can.useCompliance(currentTier))
    hints.push({
      feature: "compliance",
      requiredTier: "m10",
      description: "SOC 2 / GDPR compliance tools and audit trails",
    });
  ...
  if (!can.useSharedAgentPool(currentTier))
    hints.push({
      feature: "sharedAgentPool",
      requiredTier: "teams",
      description: "Share agents across your entire team",
    });
  if (!can.useWorkflowAutomation(currentTier))
    hints.push({
      feature: "workflowAutomation",
      requiredTier: "teams",
      description: "Chain and schedule multi-agent workflows",
    });
  if (!can.useTeamDashboard(currentTier))
    hints.push({
      feature: "teamDashboard",
      requiredTier: "teams",
      description: "Team-wide usage analytics and billing overview",
    });
  return hints;
}
