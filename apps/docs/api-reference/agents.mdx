...
Maschina stores episodic memories from each run and automatically injects relevant past context into future runs for the same agent and user. This gives your agents long-term continuity without any extra work on your end.
...
| `offset` | string | — | Pagination cursor from a previous response's `nextOffset`. |
  ...
  "points": [
    {
      ...
      "payload": {
        ...
        "run_id": "run_01def...",
        "role": "output",
        "text": "The agent's response text stored as a memory.",
...
Permanently deletes all episodic memories for this agent. This cannot be undone.
