      ...
      <div className="min-h-dvh bg-black flex flex-col">
        ...
        <main id="main-content" tabIndex={-1} className="flex-1 flex flex-col items-center justify-center px-5 pt-16 pb-16 text-center outline-none" style={{ position: "relative", zIndex: 2 }}>
          <div className="relative px-20 py-16 flex flex-col items-center">
            ...
            <p aria-hidden className="text-[120px] font-bold leading-none tracking-tighter text-[#F84242]/10 select-none" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>err</p>
            <h1 className="text-2xl font-semibold tracking-tight text-white mt-2">Something broke.</h1>
            <p className="mt-2 text-sm text-white/30">An unexpected error occurred. Try reloading the page.</p>

            {import.meta.env.DEV && this.state.error.message && (
              <p className="mt-4 text-xs text-white/20 font-mono max-w-xs break-all">{this.state.error.message}</p>
            )}
